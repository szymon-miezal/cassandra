    ...
    public void retryingFailedReplaceWithNodeInHibernateState() throws IOException
    {
        try (WithProperties properties = new WithProperties())
        {
                                          ...
                                          .start() )
            {
                ...
                // simulate a failure during node replacement by setting hibernate status for the node to replace on seed
                ...
                // then
                // replacement will succeed as seeds gossip with hibernated nodes
